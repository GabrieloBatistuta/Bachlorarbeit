...
public class Gauss3 extends AppCompatActivity {
    ...
    public void keybordversteken(View v) {
        hideSoftKeyboard(Gauss3.this, v);
    }

    public void aktivieren() {
        if (buffer.gListeIsEmpty(3)) {
            gaussAktivieren();
        } else
            gaussLesen();
    }
    ...
    public void gaussLesen() {
        EditText[][] edittextFeld = new EditText[3][4];
        Matrix m = buffer.gListeZurueckgebenABCD(3);
    ...
    public void gaussRechnen() {
        ...
        if (gibtEsFehler) {
            ...
            ErgebnisseSpeichern(vr.gaussBerechnen(buffer.gListeZurueckgebenABCD(3)));
        }
    }
    ...
    public void gaussErstellen() {
        ...
        String[][] stringFeld = new String[3][4];
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 4; j++) {
        ...
        double zahl = 0;
        Matrix m = vr.matrixErzeugen(3, 4);
